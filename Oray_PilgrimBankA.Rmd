...
bank.df$`9Age`= fct_explicit_na(bank.df$`9Age`, na_level = "0") #Adding level '0' as category for NA age

bank.df$`9Inc`= fct_explicit_na(bank.df$`9Inc`, na_level = "0") #Adding level '0' as category for NA income
...
```
As was shown in part 2, all variables were originally stored as numeric. Changing variables such as Online, Age, Income, District, and Bill Pay to factors will make for easier analysis. We also decided to use "0" as the NA category for both Age and Income.
...
inc_dist <- ggplot(bank.df, aes(x=`9Inc`, y=`9Profit`, fill=`9Inc`)) +
  geom_boxplot()+
  scale_y_log10() +
...
age_dist <- ggplot(bank.df, aes(x=`9Age`, y=`9Profit`, fill=`9Age`)) +
  geom_boxplot() +
  scale_y_log10()+
  xlab("Age Category") +
  ylab("Profitability") +
...
district_dist <- ggplot(bank.df, aes(x=`9District`, y=`9Profit`, fill=`9District`)) +
  geom_boxplot() +
  scale_y_log10()+
  xlab("District") +
  ylab("Profitability") +
...
sub_dist <- ggplot(bank.df, aes(x=subscription_grp, y=`9Profit`, fill=subscription_grp)) +
  geom_boxplot() +
  scale_y_log10()+
...
#Mean profitability for each subscription group

bank.df.mean <- bank.df %>%
  group_by(subscription_grp) %>%
  summarise(profit_mean = mean(`9Profit`), profit_sd = sd(`9Profit`))
...
n = 10000
sample_dist = sample(bank.df$`9Profit`, n)
mean(sample_dist)
sd(sample_dist)
...
t.test(bank.df$`9Profit`, bank.df.mean$profit_mean)
