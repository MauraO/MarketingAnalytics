...
```
For the missing data we created a category named ‘0’ for each of the columns (Age, Income). The same could have been checked for null hypothesis and then replaced with the mean for the data.
We have converted columns – Online banking, Age and Income as Factor Variables for an easy statistical analysis, since those variables are categorical.
The two-way frequency table in count shows us that, 27,780 customers do not use either online banking or bill pay which constitutes 88% of the total data, customers who have either are a total of 3326 and only 528 customers who do both online banking and bill pay. Thus, it is evident that the bank has a majority of customers who still bank the traditional way.
For the new variable subscription group, we have 4 categories named – 1 – customers who do neither, 2 – customers who are active on Online banking only, and 3 - customers who are active on Bill Pay only – we actually removed this category since this has zero customers 4 – customers who are active on both the channels.
