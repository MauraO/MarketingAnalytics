...
ebay_treatment = subset(ebay[order(ebay$rDate), ],isTreatmentPeriod == "1") 
head(ebay_treatment)
...
#Small decrease in revenue during treatment period (1) for treatment group (1). While this seems insignificant, we do not know the whole customer base population, so if there are a lot of customers, this revenue loss can add up. Also, we do not know the full effect of time - that is to say how this revenue decrease will add up over time. 
...
#Null hypothesis is true, we do not reject because p-value is high
```
...
ebay2 <- ebay %>%
  filter(isTreatmentPeriod == 1) #13020 observations

ebay_dma = lm(log(revenue) ~  dma + isTreatmentGroup, data = ebay2) # to discuss 
summary(ebay_dma)

ebay_dma1 = lm(log(revenue) ~ isTreatmentGroup, data = ebay2)
summary(ebay_dma1)
...
# ?????????
# Maybe clustering would help? 
# p value is very low, but coefficient also small (lots of scattered data - from DMA?)
